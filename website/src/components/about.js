...
const Form = () => {
  ...
  const sendEmail = (e) => {
    ...
    emailjs.sendForm(email_type, email_template, e.target, email_user_id).then(
      (result) => {
        let toastKey;
      ...
      <div className="contact-container">
        ...
        <form onSubmit={sendEmail}>
          <FormControl label={`${t("yourName")}`}>
            <Input placeholder={`${t("yourName")}`} name="first_name" value={first_name} onChange={(event) => setName(event.currentTarget.value)} required />
          </FormControl>
          <FormControl label={`${t("emailSubject")}`}>
            <Input placeholder={`${t("emailSubject")}`} name="last_name" value={last_name} onChange={(event) => setSubject(event.currentTarget.value)} />
          </FormControl>

          <FormControl label={`${t("yourEmail")}`} error={shouldShowError ? `${t("emailErreurMessage")}` : null}>
            <Input placeholder={`${t("yourEmail")}`} name="email" value={email} onChange={onEmailChange} onBlur={() => setIsVisited(true)} error={shouldShowError} overrides={shouldShowError ? { After: Negative } : {}} type="email" required />
          </FormControl>

          <FormControl label={`${t("emailBody")}`}>
            <Textarea placeholder={`${t("emailBody")}`} name="message" value={message} onChange={(event) => setMessage(event.currentTarget.value)} required />
          </FormControl>
